function [im_pad, imsize] = impad(im, padsize, im_margin)
%IMPAD   [im_pad, mn] = impad(im, padsize, immargin)
...
if L == 1 %2-D
    if strcmpi(padsize,'diagonal')
        D = ceil(sqrt(M^2+N^2))+2*im_margin;
        M_pad = floor((D-M)/2);
        N_pad = floor((D-N)/2);
        m = D;
        n = D;
        ...
        M_pad = ceil(padsize(1))+im_margin;
        N_pad = ceil(padsize(2))+im_margin;
        m = M+2*M_pad;
        n = N+2*N_pad;
        imsize = [m n];
    end
    ...
    if strcmpi(padsize,'diagonal')
        D = ceil(sqrt(M^2+N^2+L^2))+2*im_margin;
        M_pad = floor((D-M)/2);
        N_pad = floor((D-N)/2);
        L_pad = floor((D-L)/2);
        m = D;
        ...
        M_pad = ceil(padsize(1))+im_margin;
        N_pad = ceil(padsize(2))+im_margin;
        L_pad = ceil(padsize(3))+im_margin;
        m = M+2*M_pad;
        n = N+2*N_pad;
        l = L+2*L_pad;
        if L == 1
